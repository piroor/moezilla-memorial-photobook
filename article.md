...
しかし、Web標準技術にはW3Cの仕様や解説があっても、Mozilla独自の技術にはドキュメントが無い場合が多く、それらはFirefox自体のソースコードを見て使い方を把握するしかない状況でした。
...
Firefoxほどの自由度を特に求めていなかった客層にとっては、このくらいの気軽さの方が魅力的に感じられた、というわけです。
...
この反省を踏まえてFirefox 52で導入された3つ目の仕組みこそが、本命の「WebExtensions API」。
...
WebExtensions APIが発表された当初は、古くからの拡張機能開発者の中には「またFUELの焼き直しが来るのか？　どうせまた失敗するんじゃないのか」と懐疑的に見ていた者もいました[^webextensions-fuel]。
...
そのような性質の物である以上、APIの機能不足を補うためにはFirefox内部の深くにアクセスする必要があり、結局はFirefoxと密結合になってしまって、元の木阿弥にしかならないのではないか、と思えたのです。
...
「大変だけど、自力救済であれば何でもできる」という従来型のXULアドオンに慣れ親しんだ拡張機能開発者にとっては、これは今までの拡張機能とはまったく違う作り方だったため、WebExtensions APIに基づく拡張機能は、極めて窮屈でやりにくい物に感じられたのでした。
とはいえ、すでにChromeでの拡張機能開発に慣れ親しんだ人にとっては、これはほとんど追加の学習の必要なしに、それこそ「同じソースコードからChrome用とFirefox用のそれぞれのパッケージを作り分けられる」程度にまで、拡張機能をChromeからFirefoxへ移植する際のコストを引き下げる物ではありました。

先の飲食店の例えをまた持ち出すと、WebExtensionsは「調理場を使わせてくれていたあのラーメン屋が、定型のトッピングの提供も始めた」ようなものでした。
料理の腕を自在に振るっていた常連客達や、それ目当てだった常連客達にとっては、新しいやり方はあまり魅力的には感じられませんでしたが、料理の心得がない客も、メニューに書かれたトッピングの組み合わせだけで好みの味を模索できるようになり、店としては今までより広い客層を受け入れられるようになったというわけです。

ただ、そのように「客層を拡げる」こととは別の大きな狙いが、WebExtensions API導入の陰にはありました。
それが、「XULアドオンの将来的な廃止」という大きな決断です。
...
しかし、XULアドオンの仕組みをいきなり廃止したのでは、ユーザーが不利益を被るだけになってしまいます。
「定型のトッピングを提供し始めたばかりのあの行きつけのラーメン屋が、急に調理場を立ち入り禁止にした」ようなもので、今まで調理場で腕を振るってきた常連客も、それ目当てで来ていた常連客も、これでは途方に暮れてしまいます。
...
店としては、もう調理場を自由に使わせることはできないけれど、今まで振る舞われていた料理に近い体験はできるようにしておいてあげたい。
腕を振るっていた常連客が、定型のトッピングの組み合わせの範囲で、狙った通りの味の料理を作れるようにしておきたい。
端的に言えば、この時点ではトッピングの種類の拡充、すなわち「WebExtensions APIへの機能追加」がまだまだ必要だったということです。

ただ、FirefoxのWebExtensions APIは、Chromeの物をベースにしつつ、ニーズに応えてFirefox独自の機能も提供する方針が示されていましたが、そもそもどのようなニーズがあるのか、どのようなニーズになら応えられるのか、といった点は未知数でした。
...
充分なソフトランディングだったとは言い難かったものの[^keeping-xul]、FirefoxはついにXULアドオンからWebExtensionsへの完全移行を果たしたのでした。
...
これは料理を振る舞っていた客の立場からすると、「調理場は使えなくなってしまったので、その頃のレシピは封印した上で、今までとはまったく違うやり方で似た味を再現するしかなくなっら」ような状況です。
拡張機能の作者にとっては、前のやり方を一旦忘れて、トッピングの組み合わせだけで味を再現するやり方に切り替えるというパラダイムシフトが必要でした。
腹を括って発想を転換し、XULアドオン時代の最も重要な価値だったものを分析して、WebExtensions APIの組み合わせで近い価値を再現する方法を考えるという、今までに無かった努力が必要となったのです。
ここで振るい落とされてしまった拡張機能は残念ながら相当数あり、この大移行を「拡張機能の大量虐殺で、横暴だ」と感じた人もいたようです。
