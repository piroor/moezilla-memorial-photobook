...
しかし、ちょうどその頃リリースされたChromeのJavaScriptエンジン「V8」は、その時点ですでに、実行前の時点でプログラム全体を効率よくコンパイルする方式[^just-in-time]を採用していました。
...
結局、JITだけが理由ではないにせよ、当時実際にエンドユーザーが実用できたJavaScript実行エンジン達の中ではV8が飛び抜けた高性能を示していたこともまた、Chromeが人気を集める大きな理由となったのでした。
