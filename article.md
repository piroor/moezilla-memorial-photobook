...
必要になったタイミングで「まさにその時（Just-in-Time）」にプログラムをソースコードから機械語にコンパイルして、以後はコンパイル済みの物を使うことにより、実行性能を大きく引き上げるという技術です。

ただ、JITにはその性質上「最初の1回のコンパイル」にはインタープリタでの実行よりも時間を要しがちです。
どのように効率よくコンパイルを行うか・どの程度の最適化度合いでコンパイルするかといった判断には、ベンダー各社の方針が表れる所です。
FirefoxのJavaScriptエンジン「SpiderMonkey」では、インタープリタでの実行状況を監視して統計情報を収集し高頻度で実行される箇所からコンパイルする「Tracing JIT」方式を採用していました[^tracing-jit]。

[^tracing-jit]: FirefoxのUI自体がJavaScriptで作られていたことから、JITの導入でFirefox全体の動作がより快適になるのではないかと期待した人もいましたが、実際にはそれほど劇的な改善とはなりませんでした。これは、DOMオブジェクトやXPCOMコンポーネントオブジェクトなどが関係する処理に対してTracing JITが有効に機能しなかったことが原因だと言われています。後に、プログラムが読み込まれた時点で全体的にコンパイルする方式との併用に改良され、ようやく当初期待されたような高速化が果たされたのでした。

他方、ChromeのJavaScriptエンジン「V8」は、実行前の時点でプログラム全体をコンパイルする方式[^just-in-time]を採用していました。
他にもさまざまな高速化・最適化技術が投じられていたこともあって、V8は登場当初から、当時のJavaScript実行エンジンの中で飛び抜けた高性能を示します。
これもまた、Chromeが人気を集める大きな理由となりました。

[^just-in-time]: それなら事前コンパイルじゃないのか？と思うかもしれませんが、Webページの埋め込みリソースとしてJavaScriptのソースコードが読み込まれた「まさにその時」にコンパイルするため、これもJITなのでした。プログラム作者の側でソースコードをコンパイルしたバイナリを配布すれば、それは事前コンパイル型ということになります。
