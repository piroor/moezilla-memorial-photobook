...
Firefoxを全体的に高速化してくれるであろう新技術[^firefox-ui-optimization]ということで、実際の製品投入はまだ先の事ながら、その第一報にユーザーは大きな期待に胸を膨らませました[^tracing-jit]。
...
しかし、その報せが出てからさほど間を空けずにリリースされたChromeの初版では、その時点ですでに、実行前の時点でプログラム全体を効率よくコンパイルする方式[^just-in-time]を採用したJavaScriptエンジン「V8」が搭載されていました。
登場当初から極めて良好な性能を示していたV8に、「最初の1回のコンパイルは時間がかかるもの」という先入観を持っていた筆者のような人達は、驚きを禁じ得ませんでした[^initial-release-of-chrome]。
...
[^initial-release-of-chrome]: Chromeの初版リリースは2008年9月でした。Firefoxユーザーにとっては、「次版のFirefoxで利用可能になると予告されたばかりの先進技術」に連なる物を備えた製品が他のベンダーから、それも即座に利用可能な状態で登場したという形だったこともあり、その衝撃は一層大きかったのでした。
