...
Mozillaが採用したのは、インタープリタでの実行を監視して統計情報を収集し、特に高い頻度で実行される箇所を選択的に高い最適化度合いでコンパイルする「Tracing JIT」方式でした。
GoogleがChromeのV8エンジンで採用したのは、そういった個別の判断を行わずにほどほどの最適化度合いでとにかく全部コンパイルしてしまう「Baseline JIT」方式でした[^baseline-jit]。
どのやり方が高性能になるかは処理対象のプログラムの設計次第、つまりWebアプリケーションの作り方の流行り廃りの影響を受ける所でもあり、各ベンダーで様々なアプローチが試みられていました。
