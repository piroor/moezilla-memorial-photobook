...
この頃、Webページの動的なコンテンツの実行性能を高めるJIT[^jit]コンパイラの技術が、一般に利用可能なWebブラウザーにもたらされ始めました。

[^jit]: Just-In-Timeの略。C言語などの「使うかどうかは分からないうちからコンパイルしておく」形式に対して、「必要になったその時に」コンパイルすることから、そう呼ばれます。
...
AutoConfigまたはMCD[^mcd]と呼ばれる機能で、Firefoxのインストール先は以下の所定の位置に設定ファイルを設置しておくことで、起動時にそれが読み込まれ、Firefoxの各設定を個別に固定できるようになっています。
