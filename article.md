...
必要になったタイミングで「まさにその時（Just-in-Time）」にプログラムをソースコードから機械語に1回だけコンパイルして、以後はコンパイル済みの物を使うことにより、実行性能を大きく引き上げるという技術です。

コンパイルをいつどのように行うかにはベンダー各社で考え方がが別れます。
FirefoxのJavaScriptエンジン「SpiderMonkey」の場合、当初は、「最初の1回のコンパイル」にはインタープリタでの実行よりも時間を要しがちなことから、インタープリタでの実行状況を監視して統計情報を収集し、高頻度で実行される箇所からコンパイルする「Tracing JIT」方式を採用しました[^tracing-jit]。

[^tracing-jit]: ただ、実際にはそれほど劇的な改善とはなりませんでした。これは、DOMオブジェクトやXPCOMコンポーネントオブジェクトなどが関係する処理に対してTracing JITが有効に機能しなかったことが原因だと言われています。

一方、ChromeのJavaScriptエンジン「V8」は当初から、実行前の時点でプログラム全体を効率よくコンパイルする方式[^just-in-time]を採用していました。
登場当初から良好な性能を示していたV8に、「最初の1回のコンパイルは時間がかかるもの」という先入観を持っていた筆者のような人達は驚きを禁じ得ませんでした。
...
JITだけが理由ではないにせよ、V8は当時のJavaScript実行エンジン達の中で飛び抜けた高性能を示していました。
そのことまた、Chromeが人気を集める大きな理由となったのでした。
