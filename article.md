...
例えば、当時大きく取り沙汰された技術の1つにJITコンパイラの導入がありました。
...
どのように効率よくコンパイルを行うか・どの程度の最適化度合いでコンパイルするかの判断には、ベンダー各社の方針が表れる所です。
MozillaがFirefoxのSpiderMonkeyエンジンで採用したのは、インタープリタでの実行を監視して統計情報を収集し、特に高い頻度で実行される箇所を選択的に高い最適化度合いでコンパイルする「Tracing JIT」方式でした[^jit-in-firefox-itself]。
GoogleがChromeのV8エンジンで採用したのは、そういった個別の判断を行わずにほどほどの最適化度合いでとにかく幅広くコンパイルしてしまう「Baseline JIT」方式でした。
どのやり方が高性能になるかは処理対象のプログラムの設計次第、つまりWebアプリケーションの作り方の流行り廃りの影響を受ける所でもあり、各ベンダーで様々なアプローチが試みられました[^baseline-jit]。

[^jit-in-firefox-itself]: FirefoxのUI自体がJavaScriptで作られていたことから、このことでFirefox全体の動作がより快適になるのではないかと期待した人もいましたが、実際にはそれほど劇的な改善とはなりませんでした。これは、DOMオブジェクトやXPCOMコンポーネントなどのJavaScriptネイティブでないオブジェクトが関与する処理にはJITが働かないという、SpiderMonkeyの制約のためだったようです。
[^baseline-jit]: その過程で、当初採用した方式から別の方式への切り替えや、複数方式の混在なども行われました。実際、Mozillaも後々Baseline JITを導入し、Tracing JITとの併用でさらなる高速化を目指すことになります。
