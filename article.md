...
また、当時はまだ物珍しかった「インターネット上での商取引」にもNetscapeは目を付け、電子署名と暗号化通信の技術であるSSL（Secure Socket Layer）[^ssl]をブラウザーに組み込んで、Amazon.comのようなeコマースも可能にしました。
...
[^ssl]: 今のTLS（Transport Layer Security）。
...
現代のギャルゲーやソシャゲによく見られる「キャラクターの立ち絵が表示され、テキストと音声で物語が進行する」形式のコンテンツは、その末裔と言えるかもしれません。
...
加藤氏はその後Microsoft社に在席してInternet Explorerのメンテナンスを担当した後、今ではMozillaでFirefoxの開発に関わられています[^makoto-kato]。

[^makoto-kato]: https://jp.linkedin.com/in/makoto-kato
...
[^baseline-jit]: 後にはMozillaもBaseline JITを導入し、JavaScript実行エンジンの高性能化をますます進めていく事になります。
...
XULアドオンの行き詰まりを目の当たりにしていたMozillaの開発者達にとって、従来型アドオンの仕組みは「廃止するか、廃止しないか」を議論する対象ではなく、[もはや「いつ、どのように廃止するか」を議論するレベルの対象でした](https://piro.sakura.ne.jp/latest/blosxom/mozilla/xul/2020-08-22_why-did-mozilla-remove-xul-add-ons.htm)。
...
また性能面では、CSSのスタイル指定のレンダリング処理を効率化するQuantum CSS（Stylo）、3Dゲームエンジンの実装に使われる手法を応用したパイプライン処理によって画面描画のフレームレートを劇的に改善する[WebRender](https://hacks.mozilla.org/2017/10/the-whole-web-at-maximum-fps-how-webrender-gets-rid-of-jank/)など、Geckoとは別ラインで実験的に開発されていた新レンダリングエンジン「Servo」の成果が続々とGeckoに取り込まれました。
