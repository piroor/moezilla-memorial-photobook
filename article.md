...
そのためMozillaはFennecの開発継続を断念し、他のブラウザー製品と同様に「レンダリングエンジンのGeckoはコンテンツの処理のみに使い、UIはそのOSのネイティブアプリとして実装する」形式でモバイル版Firefoxを実装し直す新プロジェクト「Fenix」に舵を切り直す判断をします。
ピボットの結果モバイル版Firefoxの実用性は高まりましたが、通常の手段では任意の拡張機能をインストールできないなど、カスタマイズ性の面では大きく後退した状況です。
