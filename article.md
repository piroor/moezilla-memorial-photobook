...
コンパイルをいつどのように行うか、コンパイル時の最適化をどう行うかはベンダー各社で考え方が別れます。
FirefoxのJavaScriptエンジン「SpiderMonkey」の場合、「最初の1回のコンパイル」にはインタープリタでの実行よりも時間を要しがちなことから、インタープリタでの実行状況を監視して統計情報を収集し、高頻度で実行される箇所からコンパイルする「Tracing JIT」方式を採用しました。
Firefoxを全体的に高速化してくれるはず[^firefox-ui-optimization]ということで、実際の製品投入はまだ先の事ながら、その第一報にユーザーは大きな期待に胸を膨らませました[^tracing-jit]。

[^firefox-ui-optimization]: FirefoxはUI周りがJavaScriptで実装されていることから、単にWebコンテンツの利用が快適になる以上の効果が期待されました。
[^tracing-jit]: 2008年8月にアナウンスされたTracing JITは、2009年6月リリースのFirefox 3.5から利用可能になりましたが、実際には期待されたほどの劇的な性能改善とはなりませんでした。これは、DOMオブジェクトやXPCOMコンポーネントオブジェクトなどが関係する処理に対してTracing JITが有効に機能しなかったことが原因だと言われています。

しかし、その報せが出てからさほど間を空けずにリリースされたChromeの初版には、その時点ですでに、実行前の時点でプログラム全体を効率よくコンパイルする方式のJIT[^just-in-time]を採用したJavaScriptエンジン「V8」が搭載されていました。
登場当初から極めて良好な性能を示していたV8に、Mozillaの技術解説を見てJITを知り「最初の1回のコンパイルは時間がかかるもの」という先入観を持っていた筆者のような人達は、余計に驚くこととなりました[^initial-release-of-chrome]。

[^just-in-time]: それなら「事前コンパイル」じゃないのか？と思うかもしれませんが、JavaScriptのソースコードがダウンロードされ読み込まれた「まさにその時」にコンパイルするため、これもJITなのでした。SpiderMonkeyも、後にこの方向性でのJITを併用する形に改良されることとなります。なお、プログラム作者の側でソースコードをコンパイルしたバイナリを配布すれば、それは事前コンパイル型ということになります。
[^initial-release-of-chrome]: Chromeの初版リリースは2008年9月でした。Firefoxユーザーにとっては、「次版のFirefoxで利用可能になると予告されたばかりの新技術」を備えた製品がライバル製品で、それも即座に利用可能な状態でもたらされたという形だったこともあり、その衝撃は一層大きかったのでした。
...
結局、JITだけが理由ではないにせよ、当時実際にエンドユーザーが実用できたJavaScript実行エンジン達の中では、ChromeのV8が飛び抜けた高性能を示していた形となりました。
...
こうした競争を通じて様々な技術が投入されていった結果、JavaScriptの処理速度は、第二次ブラウザー戦争以前と比べて数倍から十数倍と飛躍的に向上しました。
...
[^genocide]: 「この事がFirefoxの凋落を決定づけた、FirefoxはXULアドオンを残すべきだった」「それまでのユーザー層と一蓮托生で消えていく運命だったとしても、Firefoxは既存ユーザーに寄り添い続けるべきだった」と考える人もいますが、後述の理由からFirefoxには存在し続けてもらわなくてはならない以上、緩慢な自殺を選ぶ道はあり得なかったと筆者は思っています。とはいえ、決断があまりに遅すぎて、遅きに失した感は否めませんでした。人気の絶頂の中で、将来を見越してあえてその時点での人気の源泉となっていた拡張機能の互換性を損なう英断を下せるような、強力なリーダーシップが本来Firefoxには必要だったのかも知れません。
